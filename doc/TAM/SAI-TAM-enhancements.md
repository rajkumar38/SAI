...
-------------|-----------------------------------------------------------------
 Authors     |<code> Rajkumar P R, Ravindranath C K (Marvell Technology Inc) </code>
 Status      | In review
...
```c
+    /** Bind a TAM object */
+    SAI_ACL_ACTION_TYPE_TAM_OBJECT = 0x0000003a,
...
+     * @default disabled
+     */
+    SAI_ACL_ENTRY_ATTR_ACTION_TAM_OBJECT = SAI_ACL_ENTRY_ATTR_ACTION_START + 0x3a,
+
+    /**
...
In this mode, reports are expressed as 1 in every N events. 
...
```c
+    /** Report in a microburst mode, specifying report rate(pps) and max number of reports per microburst event */
...
+     * @flags CREATE_AND_SET
+     * @default 10000
+     * @validonly SAI_TAM_REPORT_ATTR_REPORT_MODE == SAI_TAM_REPORT_MODE_PROBABILISTIC_SAMPLING
+     */
+    SAI_TAM_REPORT_ATTR_PROBABILISTIC_SAMPLE_RATE,
+
+    /**
+     * @brief Microburst report rate in packets per second
+     *
+     * @type sai_uint32_t
+     * @flags CREATE_AND_SET
+     * @default 8192
+     * @validonly SAI_TAM_REPORT_ATTR_REPORT_MODE == SAI_TAM_REPORT_MODE_MICROBURST
+     */
...
```
#### Microsburt tam report object
Example flow to create microburst report at rate=512pps, max reports (packets) per burst=1024packets.
Replace above step (3) with below sample workflow.
